import {
  transformListOfStringfiedObjectsIntoArray,
  deleteItemLOILS,
  Notify,
} from "../utils";
  ...
  switch (action.type) {
    ...
    case "DECREMENT_COUNTER":
      clocks.forEach((timer, clockID) => {
        if (activeClocksIDs.some((ID) => clockID === ID)) {
          ...
          if (millisecondsbytenInt > 0) --millisecondsbytenInt;
          else {
            if (secondInt > 0) {
              --secondInt;
          ...
          if (hasEnded) {
            ...
            Notify.run(String(clockID), soundfile);
            ...
            timer.millisecondsbyten = String(millisecondsbytenInt).padStart(
              2,
              "0"
            );
          }
    ...
    case "TOGGLE_TIMER_DELETE_LIST":
      ...
      return {
        ...state,
        toggleDeleteTimers: {
          isVisible: !toggleDeleteTimers.isVisible,
